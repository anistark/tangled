var $body = document.body,
    $wrap = document.getElementById('loading-wrap'),
...
mesh = new THREE.Mesh(new THREE.TubeGeometry(new(THREE.Curve.create(function() {}, function(percent) {
...
}))(), 200, 1.1, 2, true), new THREE.MeshBasicMaterial({
    color: 0xd1684e
    // , wireframe: true
}));
group.add(mesh);
...
ring = new THREE.Mesh(new THREE.RingGeometry(4.3, 5.55, 32), new THREE.MeshBasicMaterial({color: 0xd1684e, opacity: 0, transparent: true}));
...
function render() {
    ...
    animatestep = Math.max(0, Math.min(
        240, toend
        ? animatestep + 1
        : animatestep - 4));
